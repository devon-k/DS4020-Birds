...
def load_existing_keys():
    """Load already processed (station, date) pairs from CSV."""
...
def main():
    ...
    processed_keys = load_existing_keys()

    print(f"Loaded {len(processed_keys)} existing weather records")

    seen_files = set()
    idle_cycles = 0
    MAX_IDLE = 10  # stops after no new files for a while
    ...
    while True:
        ...
        new_files = [f for f in audio_files if f not in seen_files]

        if not new_files:
            idle_cycles += 1
            if idle_cycles >= MAX_IDLE:
                print("No new files detected. Exiting...")
                break
        ...
        for filename in new_files:
            ...
            print(f"Fetching: {station} - {date.date()}")
        ...
        if new_results:
            append_to_csv(new_results)
            print(f"Appended {len(new_results)} new rows")
